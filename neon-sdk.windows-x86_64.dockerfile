...
RUN cat > ${SDK}/bin/x86_64-windows-clang++ <<EOF
#!/usr/bin/env bash
exec ${SDK}/llvm-${LLVM_MAJOR}/bin/clang++ --target=x86_64-w64-windows-gnu \
  -fuse-ld=lld -stdlib=libstdc++ -gcc-toolchain /usr "\$@"
EOF
RUN chmod +x ${SDK}/bin/*

RUN cat > ${SDK}/toolchains/x86_64-windows-mingw-clang.cmake <<EOF
set(CMAKE_SYSTEM_NAME Windows)
set(CMAKE_SYSTEM_PROCESSOR x86_64)
set(CMAKE_C_COMPILER   "${SDK}/bin/x86_64-windows-clang++")
set(CMAKE_CXX_COMPILER "${SDK}/bin/x86_64-windows-clang++")
...
RUN printf '%s\n' \
  "export CC=${SDK}/bin/x86_64-windows-clang++" \
  "export CXX=${SDK}/bin/x86_64-windows-clang++" \
